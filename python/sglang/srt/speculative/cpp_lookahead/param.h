...
struct Param {
    ...
    size_t get_return_token_num(size_t batch_size) const
    {
        if (batch_size < batch_return_token_num.size()) {
            if (batch_return_token_num[batch_size]
                != std::numeric_limits<decltype(batch_return_token_num)::value_type>::max()) {
    ...
    std::vector<size_t> parse(const std::string &value) {
        ...
        for (const auto& seg : std::vector<std::string>(first, last)) {
            ...
            if (part.size() != 2) {
                throw std::runtime_error("failed to get config, invalid config: " + seg
                                         + ", part's size = " + std::to_string(part.size()));
            }
            std::regex                 endash_re("-");
    ...
    void resetBatchReturnTokenNum(const std::string& value) {
        batch_return_token_num = parse(value);
    }
    ...
    std::string detail()
    {
        ...
        ss << ", batch_return_token_num(" << batch_return_token_num.size() << ") = ";
        for (int i = 0; i < batch_return_token_num.size(); ++i) {
            ss << i << "|" << batch_return_token_num[i] << ",";
        }
        return ss.str();
    }
};
