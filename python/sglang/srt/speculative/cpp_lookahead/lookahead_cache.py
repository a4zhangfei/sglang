...
if __name__ == "__main__":
    ...
    cache = LookaheadCache(branch_length=12, draft_token_num=draft_token_num)
    cache.batch_put(token_ids)

    cache.synchronize()
    decoding_ids, decoding_masks = cache.batch_get([[1,2,3], [3, 44]])
    decoding_ids = decoding_ids.reshape(-1, draft_token_num)
    decoding_masks = decoding_masks.reshape(-1, draft_token_num, draft_token_num)
    logger.info(f"\n{decoding_ids=}\n{decoding_masks=}")

    for i in range(decoding_ids.shape[0]):
        leaf_paths = cache.leaf_paths_from_mask(decoding_ids[i].tolist(), decoding_masks[i].tolist())
